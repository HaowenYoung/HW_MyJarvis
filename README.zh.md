...
# 3. 用 Claude Code 打开仓库
claude .

# 4. 在 Claude Code 会话里：
# > 读 CLAUDE.md 和 setup_guide.md，然后初始化 raw/ 和 wiki/ 骨架

# 5. 累积几天 raw/ 数据后，第一次真正的操作：
# > compile
```
...
**本仓库目前包含个人数据**——日常日志、生活事件、心理画像、私有项目指针。在完成脱敏前它应保持 **private**。具体包含：

- `raw/daily/`、`raw/events/`、`raw/feedback/` —— 个人日志和纠正反馈
- `wiki/traits/`、`wiki/values/` —— 个人心理画像
- `raw/papers/`、`raw/ideas/` —— 未公开的研究方向
- `projects/*/source_path.txt` —— 私有 repo 路径
- `tools/*-config.env` —— API key（已在 `.gitignore` 中）

### 想基于 MyJarvis 搭自己的系统

1. 清空 `raw/` 下每个子目录（保留目录结构，删内容）。
2. 整个删掉 `wiki/traits/` 和 `wiki/values/`。
3. 所有 `tools/*-config.env` 改成 `*.env.example`，只留占位 key。
4. 删掉 `projects/*/source_path.txt`。
5. 检查 `raw/claude-sessions/` 和 `raw/scratch/` 是否还残留个人 context。
6. 重新过一遍 `.gitignore`，确认没有密钥漏出去。
...
单人维护的个人项目。API、schema、目录约定可能随时变。欢迎就架构思路开 issue；PR 看情况合。
